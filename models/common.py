...
class Conv(nn.Module):
    ...
    def forward_fuse(self, x):
        ...
        return self.quant(self.act(self.conv(x)))
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        if self.add:
            return self.quant(x + self.cv2(self.cv1(x)))
        else:
...
class C3(nn.Module):
    ...
    def forward(self, x):
        c1_quant = self.cv1(x)
        c2_quant = self.cv2(x)        
        c3_struct = self.cv3(self.quant(torch.cat((self.m(c1_quant), c2_quant), 1)))
        return c3_struct
...
class SPPF(nn.Module):
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")  # suppress torch 1.9.0 max_pool2d() warning
            x = self.quant(x)
            y1 = self.quant(self.m(x))
            y2 = self.quant(self.m(y1))
            return self.cv2(self.quant(torch.cat((x, y1, y2, self.quant(self.m(y2))), 1)))
